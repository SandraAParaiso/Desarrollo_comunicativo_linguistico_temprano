...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/sandra/Desktop/experimento/experimento_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text='¡Bienvenido! En esta tarea deberá seleccionar una de las imágenes',
        font='Open Sans',
    ...
    # --- Initialize components for Routine "imagenes" ---
    ...
    left_image = visual.ImageStim(
        win=win,
        name='left_image', 
        image='default.png', mask=None, anchor='center-left',
    ...
    right_image = visual.ImageStim(
        win=win,
        name='right_image', 
        image='default.png', mask=None, anchor='center-right',
        ori=0.0, pos=(0, 0), size=(0.5, 0.5),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "imagenes" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('imagenes.started', globalClock.getTime())
        sonido.setSound(sound, secs=2, hamming=True)
        sonido.setVolume(1.0, log=False)
        sonido.seek(0)
        left_image.setImage(stimulus)
        right_image.setImage(stimulustwo)
        ...
        # keep track of which components have finished
        imagenesComponents = [sonido, left_image, right_image, trial_mouse]
        for thisComponent in imagenesComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            # if sonido is starting this frame...
            if sonido.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.addData('sonido.started', tThisFlipGlobal)
                # update status
                sonido.status = STARTED
                sonido.play(when=win)  # sync with win flip
            
            # if sonido is stopping this frame...
            if sonido.status == STARTED:
                ...
                if tThisFlipGlobal > sonido.tStartRefresh + 2-frameTolerance:
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'sonido.stopped')
                    # update status
                    sonido.status = FINISHED
                    sonido.stop()
            # update sonido status according to whether it's playing
            if sonido.isPlaying:
                sonido.status = STARTED
            elif sonido.isFinished:
                sonido.status = FINISHED
            ...
            if trial_mouse.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        if gotValidClick:
                            corr = 0
                            corrAns = environmenttools.getFromNames(correct, namespace=locals())
                            for obj in corrAns:
                                # is this object clicked on?
                                if obj.contains(trial_mouse):
                                    corr = 1
                            trial_mouse.corr.append(corr)
                        if gotValidClick:
        ...
        # --- Ending Routine "imagenes" ---
        for thisComponent in imagenesComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('imagenes.stopped', globalClock.getTime())
        sonido.pause()  # ensure sound has stopped at end of Routine
        ...
        trials.addData('trial_mouse.corr', trial_mouse.corr)
        trials.addData('trial_mouse.clicked_name', trial_mouse.clicked_name)
        # the Routine "imagenes" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
